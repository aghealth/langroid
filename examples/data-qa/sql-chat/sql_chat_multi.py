...
def load_context_descriptions(engine: Engine) -> dict:
    ...
    while True:
        filepath = Prompt.ask(
            ...
            default="examples/data-qa/sql-chat/tables.json",
        )
...
def chat(opts: CLIOptions, config: DocChatAgentConfig) -> None:
    ...
    doc_agent.vecdb.set_collection("sql-chat-multi-extract", replace=True)
    print("[blue]Welcome to the sql info-extractor!")
    doc_agent.config.doc_paths = [
        "docs/procedures.json",
        "docs/surgeons.json",
    ...
    doc_task = Task(
        ...
        llm_delegate=False,
        single_round=True,
        system_message="""You are an expert on hospital information. 
        You will receive various questions about ids for procedures, surgeons
        and departments and your job is to answer them precisely with all possible ids.
        """,
    )
    ...
    database_uri = Prompt.ask(
        ...
        """,
        default="mysql://scalpey:4rT4EvAZU-xuJjyt@dev-cluster.cluster-cgj8ilbmhg0t.us-east-1.rds.amazonaws.com/scalpey",
    )
...
def main(
    ...
    chat(CLIOptions(fn_api=not tools, schema_tools=schema_tools), DocChatAgentConfig())
